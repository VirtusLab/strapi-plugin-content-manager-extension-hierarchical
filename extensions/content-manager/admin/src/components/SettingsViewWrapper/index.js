...
const SettingsViewWrapper = ({
  ...
  const getRelationNameOptions = () => {
    ...
    Object.entries(attributes).forEach(([key, value]) => {
     const { type, relationType } = value;
      if(type === 'relation' && relationType === 'oneWay' ) {
        options.push(key);
      }
    });
  ...
  // Adding single qoutes to be sure that relation switch has empty value in case of removing relation from the other part of app
    ...
    if (input.name === 'settings.relationName') {
       return getRelationNameOptions()
  ...
  const filterInputsByRelation = () => {
    ...
    const relationNameOptions = getRelationNameOptions();
    if(modifiedData.settings && !isRelationSwitchable()){
      return modifiedData.settings.relation
        ? inputs 
        : filteredInputs
    }
  ...
  const generateInputValue = (input) => {
    if(input.name === 'settings.relation'){
      return isRelationSwitchable() ? false :  get(modifiedData, input.name);
    }
    return get(modifiedData, input.name)
  }
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            >
              ...
              <div className="row">
                {filterInputsByRelation().map(input => (
                          ...
                          >
                            {description => (
                              <Input
                                ...
                                value={generateInputValue(input)}
                              />
                            )}
                          </FormattedMessage>
                        </div>
